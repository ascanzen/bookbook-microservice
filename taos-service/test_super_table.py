...
def test_dynamic_create_stable():
    Meter_dynamic = SuperModel.dynamic_create_table(
        "meterSD",
        db,
...
def test_get_supermodel_from_table():
    ...
    tabledes = Meter_dynamic.describe_table()
    print(tabledes)
    SonTable = Meter_dynamic.create_son_table(
        "sontabledynamic6", location="beijing", gid=4
    )
    m = SonTable(
        name_float=1.1,
        name_double=1.2,
...
def insertData():
    ...
    for i in range(1, 21):
        ...
        m = TableT1(
            ...
            curDouble=1 / i + 10,
            desc="g2",
            ts=datetime.datetime.now() - datetime.timedelta(hours=(21 - i)),
        )
        m.save()
    yield
...
def test_update_table_tag(insertData):
    ...
    assert (
        Meters.select("location").where(Meters.groupid == 6).one().location == "tianjin"
    )
    assert TableT.change_tag_value(**{"location": "tianjin1"})
    assert (
        Meters.select("location").where(Meters.groupid == 6).one().location
        == "tianjin1"
    )
...
def test_add_tag(insertData):
    Meters.add_tags(
        IntegerField(db_column="add_tag_1"),
        IntegerField(db_column="add_tag_4"),
        BinaryField(max_length=30, db_column="add_tag_5"),
    )
    tags = []
