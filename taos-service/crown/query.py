...
class QueryCompiler(object):
    ...
    def __init__(
        self, quote_char="'", interpolation="?", field_overrides=None, op_overrides=None
    ):
    ...
    def parse_select_query(self, query, start=1, alias_map=None):
        ...
        if query._interval:
            if query._interval_offset:
                parts.append(
                    "INTERVAL(%s,%s)" % (query._interval, query._interval_offset)
                )
            else:
    ...
    def parse_alter_database(
        self, database, keep=None, comp=None, replica=None, quorum=None, blocks=None
    ):
    ...
    def create_database(
        ...
        return " ".join(
            self.parse_create_database(
                database, safe, keep, comp, replica, quorum, blocks
            )
        )
    ...
    def alter_database(
        ...
        return " ".join(
            self.parse_alter_database(database, keep, comp, replica, quorum, blocks)
        )
...
class QueryResultWrapper(list):
    def __init__(self, model, cursor):
        ...
        super(QueryResultWrapper, self).__init__(
            [self.simple_mode(row) for row in cursor.data]
        )
...
class SelectQuery(Query):
        ...
        for item in selection:
            if isinstance(item, str):
                for field in all_selection:
                    if (
                        item.capitalize() == field.verbose_name
                        or item == field.db_column
                    ):
                        item = field
                        break
            res.append(item)
    ...
    def count(self, field=None):
        ...
        clone._select = [
            fn.Count(field if field else clone.model_class._meta.primary_key)
        ]
        # if self._group_by:
        #     res = clone.execute()
        #     if len(res) > 0:
    ...
    def apercentile(self, *fields):
        ...
        if self._group_by:
            clone._group_by = None
        clone._select = [
            out_alias_tuple_field(field, 3, fn.APERCENTILE) for field in fields
        ]
        res = clone.execute()
        if len(res) > 0:
            return res[0]
    ...
    def percentile(self, *fields):
        ...
        if self._group_by:
            clone._group_by = None
        clone._select = [
            out_alias_tuple_field(field, 3, fn.PERCENTILE) for field in fields
        ]
        res = clone.execute()
        if len(res) > 0:
            return res[0]
    ...
    def leastsquares(self, *fields):
        ...
        if self._group_by:
            clone._group_by = None
        clone._select = [
            out_alias_tuple_field(field, 4, fn.LEASTSQUARES) for field in fields
        ]
        res = clone.execute()
        if len(res) > 0:
            return res[0]
