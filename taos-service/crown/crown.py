...
class ModelOptions(object):
    def __init__(
        self, cls, database=None, db_table=None, order_by=None, primary_key=None
    ):
    ...
    def get_sorted_fields(self):
        return sorted(
            self.fields.items(),
            key=lambda v: (isinstance(v[1], PrimaryKeyField) and 1 or 2, v[1]._order),
        )
    ...
    def __new__(cls, name, bases, attrs):
        ...
        if meta:
            meta_options.update(
                ...
                if not k.startswith("_") and not isinstance(v, Field)
            )
            tags_options.update(
                (k, v)
                for k, v in meta.__dict__.items()
        ...
        if hasattr(cls, "__unicode__"):
            setattr(
                cls,
                "__repr__",
                lambda self: "<%s: %r>" % (cls.__name__, self.__unicode__()),
            )
...
class Model(metaclass=BaseModel):
    ...
    @classmethod
    def dynamic_create_table(
        cls, table_name, database=default_database, safe=True, **fields
    ):
        fields["Meta"] = type(
            "Meta", (object,), dict(database=database, db_table=table_name)
        )
        ...
        if table_name in database.get_tables():
            ...
            for i, f in enumerate(desc_fields):
                if f[3] != "TAG":
                    ...
                    if f[1] == "TIMESTAMP":
                        val = (
                            PrimaryKeyField(column_name=f[0])
                            if i == 0
                            else DateTimeField(column_name=f[0])
                        )
                    elif f[1] == "FLOAT":
    ...
    def table_exists(cls):
        return (
            cls._meta.db_table[cls._meta.db_table.index(".") + 1 :]
            in cls._meta.database.get_tables()
            or cls._meta.db_table[cls._meta.db_table.index(".") + 1 :].lower()
            in cls._meta.database.get_tables()
        )
    ...
    def get(self, expr):
        ...
        if query_expr:
            param = map(
                lambda x: '"%s"' % x
                if isinstance(x, str) or isinstance(x, datetime)
                else x,
                query_expr[1],
            )
...
class SuperModel(metaclass=BaseModel):
    ...
    @classmethod
    def dynamic_create_table(
        cls, table_name, database=default_database, safe=True, tags={}, **fields
    ):
    ...
    def create_son_table(cls, name, **kwargs):
        ...
        if primary_column_name != "ts":
            ...
            son_model._meta.fields[primary_column_name] = cls._meta.fields[
                primary_column_name
            ]
            son_model._meta.columns[primary_column_name] = cls._meta.columns[
                primary_column_name
            ]
        #############################################
    ...
    def add_tags(cls, *args):
        ...
        for value in args:
            if (
                ...
                and not isinstance(value, DateTimeField)
                and value.db_column
            ):
    ...
    def supertable_exists(cls):
        ...
        print(cls._meta.db_table[cls._meta.db_table.index(".") + 1 :])
        print(cls._meta.db_table[cls._meta.db_table.index(".") + 1 :].lower())

        print(cls._meta.database.get_supertables())

        return (
            cls._meta.db_table[cls._meta.db_table.index(".") + 1 :]
            in cls._meta.database.get_supertables()
            or cls._meta.db_table[cls._meta.db_table.index(".") + 1 :].lower()
            in cls._meta.database.get_supertables()
        )
