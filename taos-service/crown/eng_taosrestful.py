...
class Cursor(list):
    ...
    def execute(self, sql, param=(), canDict=False):
        ...
        if res:
            self.status = res.get("code")
            if self.status == 0:
                self.rowcount = res.get("rows")
                self.head = res.get("column_meta")
                if canDict:
                    for d in res.get("data") or []:
                        self.data.append(Row(d, self.head))
                else:
                    self.data = res.get("data")
                logger.debug(("result->", res.get("data"), res.get("column_meta")))
...
class Conn:
    ...
    def execute_sql(self, sql):
        reponse = requests.post(
            self.url, auth=(self.user, self.passwd), data=sql.encode()
        )
...
class TaosRestful:
    def connect(
        self, database, host="localhost", port=6041, user="root", passwd="taosdata"
    ):
