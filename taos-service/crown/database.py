...
class Database(object):
    ...
    def close(self):
        with self._conn_lock:
            if self.deferred:
                raise Exception(
                    "Error, database not properly initialized before closing connection"
                )
    ...
    def get_compiler(self):
        return self.compiler_class(
            self.quote_char, self.interpolation, self.field_overrides, self.op_overrides
        )
    ...
    def create_database(
        self, safe=False, keep=None, comp=None, replica=None, quorum=None, blocks=None
    ):
        qc = self.get_compiler()
    ...
    def alter_database(
        self, keep=None, comp=None, replica=None, quorum=None, blocks=None
    ):
        qc = self.get_compiler()
        res = self.execute_sql(
            qc.alter_database(
